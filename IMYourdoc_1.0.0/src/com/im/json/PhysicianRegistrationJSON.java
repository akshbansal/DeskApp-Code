...
public class PhysicianRegistrationJSON {
	...
	PhysicianRegisterBO physicianBO;
	public PhysicianRegistrationJSON(PhysicianRegisterBO physicianBO){
	...
	public String doPatientRegistration() {
		...
		try {
			...
			if(Constants.IS_ADMIN_LOGIN == true){
				...
				jsonObject.put("user_type", "Physician");
			}
			else{
				jsonObject.put("method", rb.getString("physician_registration"));
			...
			jsonObject.put("npi" , physicianBO.getNPI());
			jsonObject.put("practice_type" , physicianBO.getPractice_type());
			...
			jsonObject.put("other_hospitals" , physicianBO.getOther_hospitals());
