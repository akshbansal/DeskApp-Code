...
public class GroupChatPanel {
	...
	public GroupChatPanel(byte[] image, JLabel time) {
		this.image = image;
		this.time = time;
	}
	
	public JPanel getLeftBubble(final String msg, final String imageURL,final String From,final String messageId,String currGroup) throws IOException {
		...
		BufferedImage icon = Util.getProfileImg(From);
		if (icon == null) {
...
//				img = EncryptDecryptData.decodeToImage(msg);
						...
						if (value == JOptionPane.OK_OPTION) {
							...
							if (rVal == JFileChooser.APPROVE_OPTION) {
								...
								try {
									...
									fileChooser.addPropertyChangeListener(new PropertyChangeListener() {
										public void propertyChange(PropertyChangeEvent evt) {
											if (JFileChooser.DIRECTORY_CHANGED_PROPERTY.equals(evt.getPropertyName())) {
												...
												if(Constants.showConsole) if(Constants.showConsole) System.out.println("oldDir;;;" + oldDir);
												if(Constants.showConsole) if(Constants.showConsole) System.out.println("newDir;;;" + newDir);
												if(Constants.showConsole) if(Constants.showConsole) System.out.println("curDir;;;" + curDir);
									...
									fileToSave = fileChooser.getSelectedFile();
									if(Constants.showConsole) if(Constants.showConsole) System.out.println("file tosave path;;;;;" + fileToSave);
									saveFileToSystem(imageURL.trim(), fileName, fileToSave);
			...
			vbox.add(Box.createHorizontalStrut(5));
			msgPanel.add(vbox);
		} else if (null != msg.trim() && StringUtils.isEmpty(imageURL)) {
			if(Constants.currentChatWindowUSERID!=null){
				if(Constants.currentChatWindowUSERID.equals(currGroup.split("@")[0])){
					if(!From.contains("@imyourdoc.com"))
						XmppUtils.getMsgEventManager().sendDisplayedNotification(From+"@imyourdoc.com", messageId);
...
//							FileUtils.copyURLToFile(url, finalFile);
		...
		Box vbox = Box.createVerticalBox();
		vbox.add(msgPanel);
		msgPanel.addMouseListener(new MouseListener() {
	...
	public void saveFileToSystem(final String fileURL, final String filePath, final File file) throws IOException {
		...
		SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
			...
			protected Void doInBackground() throws Exception {
				...
				if(Constants.showConsole) if(Constants.showConsole) System.out.println("fileName:-----" + fileName);
				if(Constants.showConsole) if(Constants.showConsole) System.out.println("filePath:----" + filePath);
				if(Constants.showConsole) if(Constants.showConsole) System.out.println("????>>>>>>>>>" + file.getAbsolutePath());
...
class LeftBubble extends JPanel {
	...
	public void paintComponent(Graphics g) {
		try {
			img = ImageIO.read(GroupChatPanel.class.getResource("/images/black_bubble.png"));
